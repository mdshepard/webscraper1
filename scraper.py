#!/usr/bin/env python
'''
Simple Web Scraper + scraper II
Given a website url, this program finds all absolute urls, relative urls,
email addresses and phone numbers in website's html code and prints them.
'''
import sys
import re
import requests
...
def get_response(url):
    ...
    try:
        ...
        print 'Problem Encountered: {0}'.format(exc)
        sys.exit(1)
...
def scrape_phonenums(string):
    ...
    sorted_digits = sorted(set(['-'.join(d) for d in digits]))
    return '\n'.join(sorted_digits)
